...
#applies a 0.1 default rebate
fixed_rebate_example = {
  "clientId": "C2",
  "rebates": [
    {
      "type": "default",
...
#applies a 0.1 rebate per contract on a monthly basis for transactions > 5000
monthly_threshold_example = {
  "clientId": "C3",
  "rebates": [
    {
      "type": "monthly",
...
examples = [fixed_rebate_example,monthly_threshold_example,yearly_rebate_example]
...
def apply_yearly_rebate(rebate, client, df):
    ...
    df['ytd_volume'] = df.groupby(['clientId','exchange','account','salesman_code','year'])['volume'].transform('sum')
    ...
    # Conditionally calculate the volume rebated and the actual amount
    df[['volume_rebated', 'volume_rebated_amount']] = df.apply(lambda row: calculate_rebate(row, rebate.get("rebate_amount_per_contract")), axis=1)
    return df

# Function to calculate rebate
def calculate_rebate(row, rebate_amount):
    if row['eligibility']:
        volume_rebated = min(row['volume_eligible'], row['volume'])
        volume_rebated_amount = volume_rebated * rebate_amount
        return pd.Series([volume_rebated, volume_rebated_amount])
    else:
